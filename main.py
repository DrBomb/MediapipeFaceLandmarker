import argparse, sys, socket, time, json, threading
...
if platform == "win32":
    parser.add_argument("--opencv", action="store_true", help="Use OpenCV instead of Dshow as the backend")
    parser.add_argument("--cap", type=int, metavar="X", default=0, help="Override default cap selection")
    parser.add_argument("--list-caps", action="store_true", help="List all capabilities for the selected camera, then exit")

parser.add_argument('--opencv-format', nargs=3, type=int, default=[None, None, None], help="Set the OpenCV format. Required for OpenCV. Width, Height and FPS", metavar="X")
...
def main(cli_args=None):
    ...
    if platform == "win32" and not args.opencv:
    ...
    # Handle opencv backend
        ...
        camera = opencv.create_camera(args.camera, *args.opencv_format)
    ...
    with FaceLandmarker.create_from_options(options) as landmarker:
        while True:
            ...
            # Handle a processed frame if ready
            if FrameReady.is_set():
                FrameReady.clear()
                frame_delta = time.perf_counter() - frame_start
                frame_start = time.perf_counter()
                # print(FrameInfo)
                face_points = FrameInfo.face_landmarks
                face_blendshapes = FrameInfo.face_blendshapes
                face_transforms = FrameInfo.facial_transformation_matrixes
                ...
                payload = {
                    ...
                    "face_transforms": [
                        [
                            face_transforms[0][0][0],
                            face_transforms[0][0][1],
                            face_transforms[0][0][2],
                            face_transforms[0][0][3]
                        ],
                        [
                            face_transforms[0][1][0],
                            face_transforms[0][1][1],
                            face_transforms[0][1][2],
                            face_transforms[0][1][3]
                        ],
                        [
                            face_transforms[0][2][0],
                            face_transforms[0][2][1],
                            face_transforms[0][2][2],
                            face_transforms[0][2][3]
                        ],
                        [
                            face_transforms[0][3][0],
                            face_transforms[0][3][1],
                            face_transforms[0][3][2],
                            face_transforms[0][3][3]
                        ]
                    ]
                }
